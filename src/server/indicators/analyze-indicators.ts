...
export function analyzeIndicators(marketData: Pick<NormalizedMarketData, "candles" | "fundingRate" | "openInterest">): { indicators: MarketIndicators; volatilityRisk: "normal" | "high"; latestClose: number; chart: ChartPoint[] } {
  ...
  const ratios = atr.slice(-100).map((value, index) => value / marketData.candles[marketData.candles.length - 100 + index]!.close);
  ...
  const chart = marketData.candles.slice(-80).map((candle, offset) => {
    const index = marketData.candles.length - 80 + offset;
