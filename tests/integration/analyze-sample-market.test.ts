...
describe("Sample market analysis", () => {
  it.each(["15m", "1h", "4h", "1d"] as const)("analyzes %s fixture", async (timeframe) => {
    const result = await analyzeSampleMarket({ timeframe, stance: "unsure" });
  ...
  it("changes stance assessment without changing score", async () => {
    const unsure = await analyzeSampleMarket({ timeframe: "1h", stance: "unsure" });
    const long = await analyzeSampleMarket({ timeframe: "1h", stance: "long" });
